...
public class Queens {
  ...
  public static SList<Board> listOfCompletions(Board b) {
    ...
    if (q == n) {
      ...
      for (int j = 1; j <= n; j++) {
        if (!b.underAttack(i, j)) {
          list = list.append(listOfCompletions(b.addQueen(i, j)));
        }
      }
      return list;
    }
  ...
  public static void viewSolutions(SList<Board> list) {
    ...
    SList<Board> solution = list;
    while (!solution.isNull()) {
      Board board = solution.car();
      view.setQueens(board.arrangement());
      solution = solution.cdr();
    }
  }
  ...
  public static void main(String args[]) {
    if (args.length > 0) {
      ...
      SList<Board> solutions = listOfSolutions(n);
      ...
      assert numberOfSolutions(3) == 0;
      assert numberOfSolutions(4) == 2;
      assert numberOfSolutions(5) == 10;
      assert numberOfSolutions(6) == 4;
      assert numberOfSolutions(7) == 40;
      assert numberOfSolutions(8) == 92;
      assert numberOfSolutions(9) == 352;
      assert numberOfSolutions(10) == 724;
    }
  }
} // class Queens
